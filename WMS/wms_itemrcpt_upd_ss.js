...
define(['/SuiteScripts/LIBRARY/common_server_lib.js', '/SuiteScripts/LIBRARY/common_lib.js', "N/runtime", 'N/search', 'N/record', 'N/file', 'N/task', 'N/error', 'N/format'],
    ...
    (common_server_lib, common_lib, runtime, search, record, file, task, error, format) => {
        ...
        const execute = (scriptContext) => {
            ...
            // CSV保存フォルダID
            let folderId = script.getParameter({name: "custscript_itemrcpt_upd_folder_id"});

            // 受信者
            let recipients = script.getParameter({name: "custscript_itemrcpt_upd_recipients"});

            // 送信者
            let author = script.getParameter({name: "custscript_itemrcpt_upd_author"});

            // SFTP連携情報
            let connectInfoId = script.getParameter({name: "custscript_itemrcpt_upd_connectInfoId"});
            ...
            try {
                ...
                if (csvFileId) {
                    ...
                    try {
                        ...
                        mrScript.scriptId = 'customscript_wms_itemrcpt_upd_mr';
                        mrScript.deploymentId = 'customdeploy_wms_itemrcpt_upd_mr';
                        mrScript.params = {
                            'custscript_csv_fileid': csvFileId,
                            'custscript_save_fileid': saveFileId,
                            'custscript_error_fileid': errorFileId,
                            'custscript_send_mail_recipients': recipients,
                            'custscript_send_mail_author': author
                        };
                        ...
                        if (e.name == 'MAP_REDUCE_ALREADY_RUNNING') {
                            mailJson.body.push('入庫連携処理');
                            log.debug('WMS_入庫連携_プログラム - エラー :', '入庫連携処理が実行中のため、しばらくしてから、再度実行してください。');
                            common_lib.sendMail(mailJson, 'MAP_REDUCE_ALREADY_RUNNING');
                        } else {
                            err_msg = '入庫連携に失敗しました、システム管理者に連絡してください。' + err_msg;
                            mailJson.body.push('入庫連携に失敗しました、');
                            mailJson.body.push(err_msg);

                            log.debug('WMS_入庫連携_プログラム - エラー :', err_msg);
                ...
                log.debug('WMS_入庫連携_プログラム - エラー :', e.message);
