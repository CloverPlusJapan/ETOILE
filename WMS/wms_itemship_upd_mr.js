...
define(['/SuiteScripts/LIBRARY/common_server_lib.js', '/SuiteScripts/LIBRARY/common_lib.js', 'N/file', 'N/record', 'N/search', 'N/format', "N/runtime", 'N/render'],
    ...
    (common_server_lib, common_lib, file, record, search, format, runtime, render) => {
        ...
        const getInputData = (inputContext) => {
            ...
            // CSVファイル
            let csvFileId = script.getParameter({name: "custscript_itemship_upd_csv_fileid"});
        ...
        const map = (mapContext) => {
            ...
            // CSV作成内部ID 正常系
            let saveFileId = script.getParameter({name: "custscript_itemship_upd_save_fileid"});

            // CSV作成内部ID 異常系
            let errorFileId = script.getParameter({name: "custscript_itemship_upd_error_fileid"});
            ...
            if (!common_lib.dataExists('itemfulfillment', filters)) {
                ...
                if (saveFlag) {
                    itemfulRecord.save();
                    common_server_lib.csvAppendLine(itemfulfillmentNo, valueAry, errorFileId, true, appendType);
                }
            }
        }
        ...
        const summarize = (summaryContext) => {
            ...
            // 受信者
            let param_email_author = script.getParameter({name: "custscript_itemship_send_mail_author"});
            let param_email_recipients = script.getParameter({name: "custscript_itemship_send_mail_recipients"});
