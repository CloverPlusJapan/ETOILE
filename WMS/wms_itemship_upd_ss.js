...
//                let csvFileId = 9747; // テスト
                ...
                if (csvFileId) {
                    ...
                    try {
                        ...
                        if (e.name == 'MAP_REDUCE_ALREADY_RUNNING') {
                            mailJson.body.push('出荷連携処理');
                            log.debug('WMS_出荷連携_プログラム - エラー :', '出荷連携処理が実行中のため、しばらくしてから、再度実行してください。');
                            common_lib.sendMail(mailJson, 'MAP_REDUCE_ALREADY_RUNNING');
                        } else {
                            err_msg = '出荷連携に失敗しました、システム管理者に連絡してください。' + err_msg;
                            mailJson.body.push('出荷連携に失敗しました、');
                            mailJson.body.push(err_msg);

                            log.debug('WMS_出荷連携_プログラム - エラー :', err_msg);
                ...
                log.debug('WMS_出荷連携_プログラム - エラー :', e.message);
